...
**Known problems:**
- **Cross-session self-recall is measured, not yet solved.** Replaying real transcript sessions oldest-first into a growing store and probing with wording never written into any note: 57% later-probe hit rate at one note per session (`TestSelfRecallDecaysWithCorpusGrowth`, `cmd/longmemeval`). Splitting each session into several atomic typed facts instead scored worse, not better (47-52% across two runs) — a 2026-09-07 cross-reference ruled out the dedup gate as the cause (sessions with a rejected fact miss the later probe at 54.5% vs. 51.3% for clean sessions, no meaningful gap); the likely mechanism is rank dilution from ~3.5x more stored entities per session, not yet confirmed.
