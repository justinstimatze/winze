...
**Next — a raw-evidence retrieval tier, closing the authoring-required gap.** Every memory system this project has checked — winze included — retrieves over something authored first (see the opening paragraph above). [Eywa](https://arxiv.org/abs/2605.30771) reaches a workable answer from a different angle: treat raw session evidence as the source of record and extracted facts as a revisable *index* over it, never a replacement, so retrieval doesn't require inventing a claim the source didn't make. Applied here: a structurally separate evidence tier — verbatim session text, hashed, deterministically searchable — sitting underneath the typed claim graph, not feeding it. Retrieval over it returns source text, not a claim, so `CLAUDE.md`'s mirror-source-commitments discipline (which governs what may be *encoded* as a claim) doesn't apply to it — it's a new object class, not a new way to populate the existing one, and it doesn't risk repeating the one time winze's automated writing did produce a fabricated `Provenance.Quote`. Not yet scoped as an implementation plan.
