...
func ContradictionDetectionLintRule() *OpenQuestion {
	...
	return &OpenQuestion{
		...
		Blocking:   false,
		Resolution: "Resolved. llm-contradiction rule in cmd/lint using Anthropic Go SDK. Opt-in via --llm flag with --llm-max-calls budget cap. Checks claim neighborhoods for semantic contradictions.",
	}
}

// -----------------------------------------------------------------------------
// Empirical findings. These were discovered during
...
// ClaimSchemaDesign captures the claim-level schema question: what is a Claim,
// how does a sentence map to graph nodes, and how are aliases resolved? Resolved:
// claims are typed predicate instances (BinaryRelation[S,O] and UnaryClaim[S]).
