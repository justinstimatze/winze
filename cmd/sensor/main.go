...
// filterNewPapers returns papers not already seen, with non-empty IDs,
// published on or after minYear. Does not mutate the seen map.
func filterNewPapers(papers []Paper, seen map[string]bool, minYear int) []Paper {
	var out []Paper
	for _, p := range papers {
		if p.PaperID == "" || seen[p.PaperID] {
			continue
		}
		if p.Year < minYear {
			continue
	...
	for _, q := range queries {
		if useArxiv {
			...
			if err != nil {
				...
				filtered := filterNewPapers(papers, state.Seen, 2024)
				newPapers = append(newPapers, filtered...)
