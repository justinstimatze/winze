...
type finding struct {
	Kind        string   `json:"kind"`     // "duplicate" | "contradiction" | "brief_drift"
			...
			"findings": map[string]any{
				...
				"items": map[string]any{
					...
					"properties": map[string]any{
						"kind": map[string]any{
							"type":        "string",
							"enum":        []string{"duplicate", "contradiction", "brief_drift"},
							"description": "Type of rot. 'duplicate'=two entities likely refer to the same real-world thing; 'contradiction'=claims about an entity that cannot all be true; 'brief_drift'=an entity's Brief no longer matches the substance of its current claims.",
						},
...
func serializeNeighborhood(n neighborhood) string {
	...
	if len(n.asSubj) > 0 {
		...
		for _, c := range n.asSubj {
			if c.objectVar != "" {
				fmt.Fprintf(&b, "  %s --%s--> %s   (claim: %s)\n", c.subjectVar, c.predicateType, c.objectVar, c.varName)
			} else {
				fmt.Fprintf(&b, "  %s is %s   (claim: %s)\n", c.subjectVar, c.predicateType, c.varName)
			}
		}
	}
	if len(n.asObj) > 0 {
