...
func collectBiasResults(dir string, topoReport *TopologyReport, jsonOut bool, printOutput bool) BiasReport {
	...
	if jsonOut {
		...
		if err := enc.Encode(report); err != nil {
			fmt.Fprintf(os.Stderr, "dream-bias: JSON encode: %v\n", err)
		}
		return report
	}
...
func auditSurvivorshipBias(dir string) BiasAuditorResult {
	...
	var irrelevantCycles []logCycle
	for _, c := range log.Cycles {
		switch c.Resolution {
		case "irrelevant":
			irrelevant++
			resolved++
			irrelevantCycles = append(irrelevantCycles, c)
		case "challenged":
			challenged++
			resolved++
	...
	if challenged == 0 && irrelevant > 3 {
		...
		// Sample up to 5 irrelevant resolutions and check if paper titles
		// contain the hypothesis entity name — a crude heuristic for
		// "this was probably relevant but classified as noise."
		sampleSize := 5
		if len(irrelevantCycles) < sampleSize {
			sampleSize = len(irrelevantCycles)
		}
		suspects := 0
		var sampleDetails []string
		for i := 0; i < sampleSize; i++ {
			c := irrelevantCycles[i]
			hypLower := strings.ToLower(c.Hypothesis)
			// Extract key words from hypothesis name (strip common suffixes)
			hypWords := strings.Fields(strings.NewReplacer(
				"Thesis", "", "Framing", "", "Hypothesis", "", "Typology", "",
			).Replace(c.Hypothesis))
			for _, p := range c.Papers {
				titleLower := strings.ToLower(p.Title)
				for _, w := range hypWords {
					if len(w) >= 5 && strings.Contains(titleLower, strings.ToLower(w)) {
						suspects++
						sampleDetails = append(sampleDetails,
							fmt.Sprintf("  %s: paper %q matches hypothesis word %q",
								c.Hypothesis, p.Title, w))
						break
					}
				}
				_ = hypLower
		...
		conclusion := fmt.Sprintf("Zero challenges found despite %d irrelevant resolutions. "+
			"The metabolism pipeline may be systematically classifying challenges as irrelevant.", irrelevant)
		if suspects > 0 {
			conclusion += fmt.Sprintf("\n  Audit sample (%d/%d): %d potentially misclassified "+
				"(paper titles contain hypothesis keywords):\n%s",
				sampleSize, len(irrelevantCycles), suspects,
				strings.Join(sampleDetails, "\n"))
		} else {
			conclusion += fmt.Sprintf("\n  Audit sample (%d/%d): 0 potentially misclassified "+
				"— paper titles don't match hypothesis keywords (probably genuinely irrelevant).",
				sampleSize, len(irrelevantCycles))
		}
