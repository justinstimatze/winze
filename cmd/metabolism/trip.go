...
func runTrip(dir string, temperature float64, promptType string, nPairs int, dryRun bool, jsonOut bool) {
...
func exprIdent(e ast.Expr) string {
	...
	case *ast.CompositeLit:
		return compositeTypeName(e.(*ast.CompositeLit))
	}
	return ""
}
...
func parseTripResponse(response string) TripConnection {
	...
	for _, line := range strings.Split(response, "\n") {
		...
		if v, ok := strings.CutPrefix(line, "CONNECTION:"); ok {
			...
			if len(v) > 0 && v[0] >= '1' && v[0] <= '5' {
				conn.Score = int(v[0] - '0')
			}
