...
func main() {
	...
	autoResolvePending := flag.Bool("autoresolve", false, "resolve pending sensor cycles with 3+ signal cycles using the current llmResolve prompt (needs ANTHROPIC_API_KEY)")
	...
	tripMinScore := flag.Int("min-score", 4, "with --trip --promote: minimum score to promote. Returned to 4 (2026-04-27) after introducing the critic as the quality gate; the previous bump to 5 over-suppressed generation. Critic now does the filtering, threshold admits candidates.")
	...
	if *autoResolvePending {
		...
		fmt.Printf("[autoresolve] resolved %d hypotheses\n", len(results))
		for _, r := range results {
			fmt.Printf("  %s → %s\n", r.Hypothesis, r.Outcome)
		}
		return
	}
...
func runSuggest(dir string) {
	...
	fmt.Printf("// then: go build ./... && go run ./cmd/lint corpus\n")
