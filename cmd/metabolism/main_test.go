...
func TestParsePhases(t *testing.T) {
	cases := []struct {
		...
		{name: "single phase", in: "bias",
			wantHas: []string{"bias"},
			wantNot: []string{"sense", "resolve", "ingest", "trip", "dream", "calibrate", "reify"}},
		{name: "tier-1 sense-only excludes reify (no tracked-file write)", in: "sense,bias,dream,calibrate",
			wantHas: []string{"sense", "bias", "dream", "calibrate"},
			wantNot: []string{"reify", "resolve", "ingest", "trip"}},
		{name: "reify is a known phase", in: "reify",
			wantHas: []string{"reify"},
			wantNot: []string{"calibrate", "bias"}},
